...
data_set = load_dataset("json", data_files="TrainingSet/newDataset.jsonl", split="train")
...
configurations = [
    ...
    #     save_total_limit=2,
    # ),
    # lower learning rate (brought to round 2 of training)
    SFTConfig(
    ...
    # ),
    # larger effective batch (brought to round 2 of training)
    SFTConfig(
        learning_rate=2.0e-4,
        num_train_epochs=3,
...
for i, training_set in enumerate(configurations):
    output_dir  = f"Checkpoints3/config_{i}"

    if (Path(f"Training Round 3/Set_{i}")).exists():
            continue
    ...
    trainer.save_model(f"Training Round 3/Set_{i}")